...
LVGLFontData::LVGLFontData(const QString &name, const QString &codeName,
                           uint8_t size, const lv_font_t *font)
...
LVGLFontData::~LVGLFontData() {
  if (m_customFont) {
    lv_font_fmt_txt_dsc_t *fdsc =
        reinterpret_cast<lv_font_fmt_txt_dsc_t *>(m_font->dsc);
...
LVGLFontData *LVGLFontData::parse(const QString &fileName, uint8_t size,
  ...
  if (error == FT_Err_Unknown_File_Format) {
    ...
    qCritical() << "LVGLFontData: Error loading font:"
                << FT_Error_String(error);
    return nullptr;
  }

  // QString unicode_start_str = "U+" + QString::number(unicodeFirst,
  // 16).rightJustified(4, '0'); QString unicode_last_str  = "U+" +
  // QString::number(unicodeLast, 16).rightJustified(4, '0');
  ...
  lv_font_fmt_txt_glyph_dsc_t *glyph_dsc =
      new lv_font_fmt_txt_glyph_dsc_t[glyphs + 1];
  ...
  uint32_t counter = 0;
  for (uint32_t unicodeAct = unicodeFirst; unicodeAct <= unicodeLast;
       ++unicodeAct) {
      ...
      for (uint j = 0; j < buf_size; ++j) {
        *buf_out = 0;
        for (int i = 0; i < 8; i += bpp) {
          ...
          *buf_out |= ((val & mask) >> i);
          if (ok) ++buf_in;  // avoid pointing to not allocated memory
        }
        ++buf_out;  // avoid pointing to not allocated memory
      }
    }

    lv_font_fmt_txt_glyph_dsc_t &desc =
        glyph_dsc[unicodeAct - unicodeFirst + 1];
    ...
    // qDebug() << QString::number(unicodeAct, 16).rightJustified(4, '0') <<
    // QChar(unicodeAct);
  }
  ...
  // font->line_height = uint8_t(std::round((face->ascender - face->descender)
  // / 64.0)); font->base_line = uint8_t(-std::round(face->descender / 64.0));
  ...
  LVGLFontData *ret = new LVGLFontData(
      name, name.toLower().replace(" ", "_").replace("-", "_"), size, font);
...
const lv_font_t *LVGLFontData::font() const { return m_font; }

QString LVGLFontData::name() const { return m_name; }

QString LVGLFontData::codeName() const { return m_codeName; }
...
bool LVGLFontData::saveAsCode(const QString &fileName) const {
  ...
  for (uint32_t unicodeAct = unicodeFirst; unicodeAct <= unicodeSec;
       ++unicodeAct) {
    stream << "\t/* U+"
           << QString::number(unicodeAct, 16).rightJustified(4, '0') << " \""
           << QChar(unicodeAct) << "\" */";
    const lv_font_fmt_txt_glyph_dsc_t &gdsc =
        fdsc->glyph_dsc[unicodeAct - unicodeFirst + 1];
  ...
  stream << "static const lv_font_fmt_txt_glyph_dsc_t " << output_name
         << "_glyph_dsc[] =\n{\n";
  stream << "\t{.bitmap_index = 0, .adv_w = 0, .box_w = 0, .box_h = 0, .ofs_x "
            "= 0, .ofs_y = 0} /* id = 0 reserved */,\n";
  const lv_font_fmt_txt_glyph_dsc_t *gdsc = fdsc->glyph_dsc + 1;
  for (uint32_t unicodeAct = unicodeFirst; unicodeAct <= unicodeSec;
       ++unicodeAct) {
    stream << "\t{.bitmap_index = " << gdsc->bitmap_index
           << ", .adv_w = " << gdsc->adv_w << ", .box_w = " << gdsc->box_w
           << ", .box_h = " << gdsc->box_h << ", .ofs_x = " << gdsc->ofs_x
           << ", .ofs_y = " << gdsc->ofs_y << "}";
    stream << " /* U+" << QString::number(unicodeAct, 16).rightJustified(4, '0')
           << " \"" << QChar(unicodeAct) << "\" */";
    if (unicodeAct != unicodeSec) stream << ",";
    stream << "\n";
    ++gdsc;
  ...
  stream << "static const lv_font_fmt_txt_cmap_t " << output_name
         << "_cmaps[] =\n{\n";
  stream << "\t{\n";
  ...
  stream << "\t\t.unicode_list = NULL, .glyph_id_ofs_list = NULL, .list_length "
            "= 0, .type = LV_FONT_FMT_TXT_CMAP_FORMAT0_TINY\n";
  stream << "\t}\n";
  stream << "};\n\n";
  ...
  stream << "static lv_font_fmt_txt_dsc_t " << output_name
         << "_font_dsc = \n{\n";
  ...
  stream << "\t.get_glyph_dsc = lv_font_get_glyph_dsc_fmt_txt,    /* Function "
            "pointer to get glyph's data */\n";
  stream << "\t.get_glyph_bitmap = lv_font_get_bitmap_fmt_txt,    /* Function "
            "pointer to get glyph's bitmap */\n";
  stream << "\t.line_height = " << m_font->line_height
         << ",          /* The maximum line height required by the font */\n";
  stream
      << "\t.base_line = " << m_font->base_line
      << ",             /* Baseline measured from the bottom of the line */\n";
  stream << "\t.subpx = LV_FONT_SUBPX_NONE,\n";
  stream << "\t.dsc = &" << output_name
         << "_font_dsc           /* The custom font data. Will be accessed by "
...
bool LVGLFontData::isCustomFont() const { return m_customFont; }

QString LVGLFontData::fileName() const { return m_fileName; }

QJsonObject LVGLFontData::toJson() const {
  QJsonObject object(
      {{"name", m_name}, {"code", m_codeName}, {"size", m_size}});
  if (m_customFont) {
    ...
    object.insert("end", int(fdsc->cmaps[0].range_start +
                             fdsc->cmaps[0].range_length - 1));
  }
  return object;
}

uint8_t LVGLFontData::size() const { return m_size; }
