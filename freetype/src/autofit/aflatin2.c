...
af_latin2_metrics_init_widths(AF_LatinMetrics metrics, FT_Face face) {
...

  {
    ...
    if (glyph_index == 0) goto Exit;

    error = FT_Load_Glyph(face, glyph_index, FT_LOAD_NO_SCALE);
    if (error || face->glyph->outline.n_points <= 0) goto Exit;
  ...
  for (bb = 0; bb < AF_LATIN_BLUE_MAX; bb++) {
    ...
    for (; p < limit && *p; p++) {
      ...
      if (best_point >= 0) {
        ...
        do {
          prev = start - 1;
          if (prev < best_first) prev = best_last;

          dist = FT_ABS(points[prev].y - best_y);
          /* accept a small distance or a small angle (both values are */
          /* heuristic; value 20 corresponds to approx. 2.9 degrees)   */
          if (dist > 5)
            if (FT_ABS(points[prev].x - best_x) <= 20 * dist) break;
        ...
        do {
          next = end + 1;
          if (next > best_last) next = best_first;

          dist = FT_ABS(points[next].y - best_y);
          if (dist > 5)
            if (FT_ABS(points[next].x - best_x) <= 20 * dist) break;
    ...
    blue->flags = 0;
    if (AF_LATIN_IS_TOP_BLUE(bb)) blue->flags |= AF_LATIN_BLUE_TOP;
     ...
     * letters.
     */
    if (AF_LATIN_IS_X_HEIGHT_BLUE(bb)) blue->flags |= AF_LATIN_BLUE_ADJUSTMENT;
...
static void af_latin2_metrics_scale_dim(AF_LatinMetrics metrics,
  ...
  if (axis->org_scale == scale && axis->org_delta == delta) return;
...
FT_LOCAL_DEF(void)
af_latin2_get_standard_widths(AF_LatinMetrics metrics, FT_Pos* stdHW,
                              FT_Pos* stdVW) {
  if (stdHW) *stdHW = metrics->axis[AF_DIMENSION_VERT].standard_width;

  if (stdVW) *stdVW = metrics->axis[AF_DIMENSION_HORZ].standard_width;
}
    ...
    for (;;) {
      ...
      /* on-curve points.                                 */
      {
        ...
        for (; pt != last; f0 = f1) {
          ...
          if (pt == last) segment->flags |= AF_EDGE_ROUND;
        }
      }
...
af_latin2_hints_link_segments(AF_GlyphHints hints, AF_Dimension dim) {
  ...
  if (len_threshold == 0) len_threshold = 1;
  ...
  for (seg1 = segments; seg1 < segment_limit; seg1++) {
...
#endif
    {
...

      {
        ...
        if (min < seg2->min_coord) min = seg2->min_coord;

        if (max > seg2->max_coord) max = seg2->max_coord;
...
af_latin2_hints_compute_edges(AF_GlyphHints hints, AF_Dimension dim) {
  ...
  if (edge_distance_threshold > 64 / 4) edge_distance_threshold = 64 / 4;
   ...
   */
  {
    ...
    for (edge = edges; edge < edge_limit; edge++) {
      ...
      do {
        ...
        /* be ignored                                                   */
        is_serif =
            FT_BOOL(seg->serif && seg->serif->edge && seg->serif->edge != edge);
        ...
        if ((seg->link && seg->link->edge) || is_serif) {
          ...
          if (edge2) {
            ...
            if (seg_delta < edge_delta) edge2 = seg2->edge;
          } else
            edge2 = seg2->edge;
      ...
      if (is_round > 0 && is_round >= is_straight) edge->flags |= AF_EDGE_ROUND;
      ...
      if (edge->serif && edge->link) edge->serif = NULL;
    }
  }
...
static void af_latin2_hints_compute_blue_edges(AF_GlyphHints hints,
  ...
  if (best_dist0 > 64 / 2) best_dist0 = 64 / 2;
  ...
  for (; edge < edge_limit; edge++) {
    ...
    for (bb = 0; bb < AF_LATIN_BLUE_MAX; bb++) {
      ...
      if (!(blue->flags & AF_LATIN_BLUE_ACTIVE)) continue;
    ...
    if (best_blue) edge->blue_edge = best_blue;
  }
}
...
static FT_Error af_latin2_hints_init(AF_GlyphHints hints,
  ...
  if (mode == FT_RENDER_MODE_MONO) other_flags |= AF_LATIN_HINTS_MONO;
...
static FT_Pos af_latin2_snap_width(AF_Width widths, FT_UInt count,
                                   FT_Pos width) {
  FT_UInt n;
  FT_Pos best = 64 + 32 + 2;
  ...
  if (width >= reference) {
    if (width < scaled + 48) width = reference;
  } else {
    if (width > scaled - 48) width = reference;
  }
...
static FT_Pos af_latin2_compute_stem_width(AF_GlyphHints hints,
  ...
  if (!AF_LATIN_HINTS_DO_STEM_ADJUST(hints) || axis->extra_light) return width;
...
static void af_latin2_hint_edges(AF_GlyphHints hints, AF_Dimension dim) {
  ...
  if (dim == AF_DIMENSION_VERT && AF_HINTS_DO_BLUES(hints)) {
    for (edge = edges; edge < edge_limit; edge++) {
      ...
      if (edge->flags & AF_EDGE_DONE) continue;
  ...
  for (edge = edges; edge < edge_limit; edge++) {
    ...
    if (!anchor) {
      ...
      anchor_drift =
          ((anchor->pos - anchor->opos) + (edge2->pos - edge2->opos)) >> 1;
      ...
      FT_TRACE5(
          ("ALIGN: left=%.2f right=%.2f ", org_left / 64.0, org_right / 64.0));
    ...
    for (edge = edges; edge < edge_limit; edge++) {
      ...
      if (delta < 64 + 16) {
        ...
        for (before = edge - 1; before >= edges; before--)
          if (before->flags & AF_EDGE_DONE) break;

        for (after = edge + 1; after < edge_limit; after++)
          if (after->flags & AF_EDGE_DONE) break;
      ...
      if (edge > edges && edge->pos < edge[-1].pos) edge->pos = edge[-1].pos;
...
static FT_Error af_latin2_hints_apply(FT_UInt glyph_index, AF_GlyphHints hints,
                                      FT_Outline* outline,
                                      AF_LatinMetrics metrics) {
  FT_Error error;
  int dim;
