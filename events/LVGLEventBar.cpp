...
LVGLEventBar::LVGLEventBar() {}

void LVGLEventBar::eventRun(lv_obj_t *obj) {
  ...
  int min = lv_bar_get_min_value(targert);
  int max = lv_bar_get_max_value(targert);
  ...
  if (way == "Not use") {
    ...
    if (proprety == "Value")
      lv_bar_set_value(targert, value, LV_ANIM_OFF);
    else
      lv_bar_set_value(targert, value, LV_ANIM_ON);
  } else {
...
QStringList LVGLEventBar::objCode(const QString &objName) {
  QStringList list;
  QString str =
...
QStringList LVGLEventBar::eventCode() {
  ...
  if (way == "Not use") {
    if (proprety == "Value")
      list << QString("lv_bar_set_value(%1_ev_%2,%3,LV_ANIM_OFF);\n")
                  .arg(name)
                  .arg(tabindex)
                  .arg(m_result.at(7));
    else
      list << QString("lv_bar_set_value(%1_ev_%2,%3,LV_ANIM_ON);\n")
                  .arg(name)
                  .arg(tabindex)
                  .arg(m_result.at(7));
  } else {
    ...
    if (proprety == "Value")
      list << QString("lv_bar_set_value(%1_ev_%2,value,LV_ANIM_OFF);\n")
                  .arg(name)
                  .arg(tabindex);
    else
      list << QString("lv_bar_set_value(%1_ev_%2,value,LV_ANIM_ON);\n")
                  .arg(name)
                  .arg(tabindex);
  }
