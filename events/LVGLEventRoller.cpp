...
LVGLEventRoller::LVGLEventRoller() {}

void LVGLEventRoller::eventRun(lv_obj_t *obj) {
  ...
  if (proprety == "Selected") {
    if (way == "Not use") {
      int index = m_result[7].toInt();
      auto max = lv_roller_get_option_cnt(targert);
      if (index > max) index = max - 1;
      lv_roller_set_selected(targert, index, LV_ANIM_OFF);
    } else {
    }
  } else if (proprety == "Selected with anim") {
    if (way == "Not use") {
      int index = m_result[7].toInt();
      auto max = lv_roller_get_option_cnt(targert);
      if (index > max) index = max - 1;
      lv_roller_set_selected(targert, index, LV_ANIM_ON);
    } else {
    }
  }
}

QStringList LVGLEventRoller::eventCode() {
  QStringList list;
  ...
  if (proprety == "Selected") {
    if (way == "Not use") {
      ...
      list << "\t\t"
           << QString("uint16_t max = lv_roller_get_option_cnt(%1_ev_%2);\n")
                  .arg(name)
                  .arg(tabindex);
      list << "\t\t" << QString("if (index > max) index = max - 1;\n");
      list << "\t\t"
           << QString("lv_roller_set_selected(%1_ev_%2, index, LV_ANIM_OFF);\n")
                  .arg(name)
                  .arg(tabindex);
    } else {
    }
    ...
    if (way == "Not use") {
      ...
      list << "\t\t" << QString("if (index > max) index = max - 1;\n");
      list << "\t\t"
           << QString("lv_roller_set_selected(%1_ev_%2, index, LV_ANIM_ON);\n")
                  .arg(name)
                  .arg(tabindex);
    } else {
    }
