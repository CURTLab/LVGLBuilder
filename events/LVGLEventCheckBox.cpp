...
LVGLEventCheckBox::LVGLEventCheckBox() {}

void LVGLEventCheckBox::eventRun(lv_obj_t *obj) {
  ...
  if (way == "Click") {
    bool b = lv_checkbox_is_checked(targert);
    lv_checkbox_set_checked(targert, !b);
  } else if (way == "On") {
    lv_checkbox_set_checked(targert, true);
  } else if (way == "Off") {
    lv_checkbox_set_checked(targert, false);
  }
}

QStringList LVGLEventCheckBox::eventCode() {
  QStringList list;
  ...
  if (way == "Click") {
    list << QString("bool b = lv_checkbox_is_checked(%1_ev_%2);\n")
                .arg(name)
                .arg(tabindex);
    list << "\t\t"
         << QString("lv_checkbox_set_checked(%1_ev_%2, !b);\n")
                .arg(name)
                .arg(tabindex);
  } else if (way == "On") {
    list << QString("lv_checkbox_set_checked(%1_ev_%2, true);\n")
                .arg(name)
                .arg(tabindex);
  } else if (way == "Off") {
    list << QString("lv_checkbox_set_checked(%1_ev_%2, false);\n")
                .arg(name)
                .arg(tabindex);
  }
