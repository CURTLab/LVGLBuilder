...
LVGLEventLed::LVGLEventLed() {}

void LVGLEventLed::eventRun(lv_obj_t *obj) {
  ...
  QString way = m_result[6];
  if (way == "Click") {
    bool b = lv_led_get_bright(targert) == LV_LED_BRIGHT_MAX ? true : false;
    if (b)
      lv_led_off(targert);
    else
      lv_led_on(targert);
...
QStringList LVGLEventLed::eventCode() {
  QStringList list;
  ...
  if (way == "Click") {
    list << QString(
                "bool b = lv_led_get_bright(%1_ev_%2) == LV_LED_BRIGHT_MAX ? "
                "true : false;\n")
                .arg(name)
                .arg(tabindex);
    list << "\t\t"
         << QString("if (b) lv_led_off(%1_ev_%2);\n").arg(name).arg(tabindex);

    list << "\t\t"
         << QString("else lv_led_on(%1_ev_%2);\n").arg(name).arg(tabindex);

  } else if (way == "On") {
    list << QString("lv_led_on(%1_ev_%2);\n").arg(name).arg(tabindex);
  } else if (way == "Off") {
    list << QString("lv_led_off(%1_ev_%2);\n").arg(name).arg(tabindex);
  }
