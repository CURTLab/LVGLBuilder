...
void LVGLItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *option,
                     QWidget *widget) {
  Q_UNUSED(option)
  Q_UNUSED(widget)
  if (m_object) {
...
QVariant LVGLItem::itemChange(QGraphicsItem::GraphicsItemChange change,
                              const QVariant &value) {
  if (change == ItemPositionChange && scene() && m_object &&
      m_object->isMovable()) {
    QPoint newPos = value.toPoint();
    QPoint absPos = m_object->absolutePosition();
    if (newPos == absPos) return absPos;
    QRect rect;
    QPoint offset(0, 0);
...
LVGLObject *LVGLItem::object() const { return m_object; }

void LVGLItem::setObject(LVGLObject *obj) {
  if (obj == m_object) return;

  if (m_object) {
    disconnect(m_object, &LVGLObject::positionChanged, this,
...
bool LVGLItem::isManipolating() const { return m_direction.any(); }
