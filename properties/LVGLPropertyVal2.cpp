...
template <class T>
LVGLPropertyVal2<T>::LVGLPropertyVal2(
...
QVariant LVGLPropertyVal2<T>::value(LVGLObject *obj) const {
  return QString("[%1,%2]")
      .arg(m_getter1(obj->obj()))
      .arg(m_getter2(obj->obj()));
}
...
void LVGLPropertyVal2<T>::setValue(LVGLObject *obj, QVariant value) {
  if (value.type() == QVariant::Map) {
    ...
    m_setter(obj->obj(), map[m_item1->name().toLower()].toInt(),
             map[m_item2->name().toLower()].toInt());
  }
}
...
LVGLPropertyVal2Int16::LVGLPropertyVal2Int16(
    int16_t min1, int16_t max1, QString value1,
    std::function<int16_t(lv_obj_t *)> getter1, int16_t min2, int16_t max2,
...
LVGLPropertyVal2UInt16::LVGLPropertyVal2UInt16(
    ...
    std::function<uint16_t(lv_obj_t *)> getter1, uint16_t min2, uint16_t max2,
