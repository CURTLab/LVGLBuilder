...
LVGLPropertyPointsDialog::LVGLPropertyPointsDialog(QWidget *parent)
  ...
  m_list->setHeaderLabels(QStringList() << "x"
                                        << "y");
...
QWidget *LVGLPropertyPointsDelegate::createEditor(
...
void LVGLPropertyPointsDelegate::setEditorData(QWidget *editor,
                                               const QModelIndex &index) const {
  if (index.isValid()) {
...
void LVGLPropertyPointsDelegate::setModelData(QWidget *editor,
...
bool LVGLPropertyPoints::hasEditor() const { return true; }
...
QVariant LVGLPropertyPoints::value(LVGLObject *obj) const {
  ...
  for (const lv_point_t &p : list) ret << QString("(%1,%2)").arg(p.x).arg(p.y);
  return ret.join(", ");
}
