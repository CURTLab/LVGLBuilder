...
LVGLObjectModel::LVGLObjectModel(QObject *parent)
    : QAbstractItemModel(parent), m_current(nullptr) {}

QVariant LVGLObjectModel::headerData(int section, Qt::Orientation orientation,
                                     int role) const {
...
QModelIndex LVGLObjectModel::index(int row, int column,
...
QVariant LVGLObjectModel::data(const QModelIndex &index, int role) const {
  ...
  const LVGLObject *o =
      reinterpret_cast<const LVGLObject *>(index.internalPointer());
...
void LVGLObjectModel::endInsertObject() { endInsertRows(); }
...
void LVGLObjectModel::endRemoveObject() { endRemoveRows(); }
...
void LVGLObjectModel::setCurrentObject(LVGLObject *obj) {
  ...
  m_current = obj;
  emit dataChanged(objIndex(obj, 0), objIndex(obj, 1),
                   QVector<int>({Qt::FontRole}));
  emit dataChanged(objIndex(old, 0), objIndex(old, 1),
                   QVector<int>({Qt::FontRole}));
}

QModelIndex LVGLObjectModel::objIndex(LVGLObject *obj, int col) const {
  if (obj == nullptr) return QModelIndex();
