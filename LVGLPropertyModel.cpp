...
QVariant LVGLPropertyModel::headerData(int section, Qt::Orientation orientation, int role) const
{
...
QModelIndex LVGLPropertyModel::index(int row, int column, const QModelIndex &parent) const
{
	if (!hasIndex(row, column, parent) || (m_obj == nullptr))
		return QModelIndex();

	if (parent.isValid()) {
		const LVGLProperty *p = static_cast<const LVGLProperty*>(parent.internalPointer());
...
{
	if (!index.isValid() || (m_obj == nullptr))
		return QModelIndex();

	const LVGLProperty *item = static_cast<const LVGLProperty*>(index.internalPointer());
...
{
	if ((m_obj == nullptr) || !index.isValid())
		return QVariant();
...
LVGLObject *LVGLPropertyModel::object() const
{
	return m_obj;
}
...
QModelIndex LVGLPropertyModel::propIndex(const LVGLProperty *prop, const LVGLWidget *widget, int column)
{
...
LVGLPropertyDelegate::LVGLPropertyDelegate(QObject *parent) : QStyledItemDelegate (parent)
{
}

QWidget *LVGLPropertyDelegate::createEditor(QWidget *parent, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
	Q_UNUSED(option)
